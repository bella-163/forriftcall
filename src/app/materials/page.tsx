      ...
      <main className="mx-auto max-w-5xl px-5 py-16 lg:px-8">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2">
          {data.items.map((item) => {
            ...
            return (
              <Link key={item.slug} href={`/materials/${item.slug}`} className={`group rounded-2xl border p-6 backdrop-blur transition hover:-translate-y-1 ${s.card}`}>
                {item.image && <img src={item.image} alt={item.name} className="mb-4 h-16 w-16 rounded-xl object-contain bg-black/25" />}
