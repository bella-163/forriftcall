...
export default function AdminDungeonsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/data?key=dungeons").then((r) => r.json()).then(setData);
  }, []);
  ...
  async function deleteItem(slug: string) {
    ...
    await fetch("/api/admin/data?key=dungeons", {
      method: "PUT",
    ...
    <div className="mx-auto max-w-4xl px-5 py-16">
      <div className="mb-10 flex items-center justify-between">
        <div>
          ...
          <h1 className="text-3xl font-black text-white">副本攻略管理</h1>
        </div>
        <Link href="/admin/dungeons/new" className="rounded-xl bg-rift-crimson px-5 py-2.5 text-sm font-black text-white hover:bg-rift-crimson/85">
          + 新增副本
        </Link>
      </div>
      {!data ? (
        ...
        <div className="space-y-3">
          {data.items.map((item) => (
            <div key={item.slug} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              ...
              <Link href={`/admin/dungeons/${item.slug}`} className="rounded-lg border border-white/15 bg-white/5 px-4 py-2 text-sm font-bold text-white hover:bg-white/10">編輯</Link>
