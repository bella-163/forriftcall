...
export function CatalogListAdmin({ dataKey, title, adminHref, newLabel }: Props) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-16">
      <div className="mb-10 flex items-center justify-between">
        ...
        <Link href={`${adminHref}/new`} className="rounded-xl bg-rift-crimson px-5 py-2.5 text-sm font-black text-white hover:bg-rift-crimson/85">
          + {newLabel}
        </Link>
      </div>
          ...
          {data.items.map((item) => (
            <div key={item.slug} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              {item.image ? (
                ...
                <div className="h-12 w-12 rounded-lg border border-white/10 bg-white/[0.03]" />
              )}
